"""
NEXUS Chat — Secure Messaging Backend (Railway-Fixed)
Flask + SocketIO + SQLAlchemy + JWT + Bcrypt + Fernet AES-128

Fixes applied:
  1. BASE_DIR — reliable static file paths (root cause of 404 on Railway)
  2. seed() at import time — DB init works with gunicorn AND python app.py
  3. /health endpoint — Railway health check
  4. create_room returns _room_payload — full data for instant room open
  5. try/except + rollback on every DB route
  6. Longer room codes — no collision on hex[:12]
"""
...
# FIX 1: Use absolute path from this file — CWD on Railway can differ
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

app = Flask(__name__, static_folder=None)

DATABASE_URL = os.environ.get("DATABASE_URL",
    f"sqlite:///{os.path.join(BASE_DIR, 'nexus.db')}")
...
if _fenv:
    FERNET_KEY = _fenv.encode()
else:
    KEY_FILE = os.path.join(BASE_DIR, ".nexus.key")
...
AVATAR_COLORS = ["#0BBCE8","#7C3AED","#10B981","#F59E0B","#EF4444","#06B6D4","#EC4899"]
MAX_TEXT_LEN  = 4000
MAX_MEDIA_LEN = 7 * 1024 * 1024
...
class Member(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("room_id","user_id", name="uq_member_room_user"),
    )
...
class Message(db.Model):
    ...
    def read(self):
        try: return cipher.decrypt(self.body.encode()).decode()
        except: return ""
    def d(self):
        u = db.session.get(User, self.user_id)
        return {"id":self.id,"room_id":self.room_id,"user":u.d() if u else None,
                "kind":self.kind,"text":self.read(),"duration":self.duration,
                "created_at":self.created_at.isoformat(),"edited":self.edited,
                "reply_to":self.reply_to}

# FIX 2: seed() at module level — gunicorn workers also get DB initialized
def seed():
...
seed()   # <── runs at import, not just inside if __name__ == "__main__"
...
@app.route("/health")   # FIX 3: Railway health check
def health():
...
def _room_payload(r, uid, m):
    ...
    if r.kind == "dm" and r.dm_key:
        ...
        if other:
            ...
            rd["emoji"] = ""
            rd["peer"]  = other.d()
    lm = Message.query.filter_by(room_id=r.id).order_by(Message.created_at.desc()).first()
    if lm:
    ...
    rd["unread"] = Message.query.filter(
...
def list_rooms():
    ...
    out.sort(key=lambda x: x.get("preview_at") or x.get("created_at") or "", reverse=True)
    return jsonify(out)
...
def create_room():
    ...
    if kind not in ("group","channel"): kind = "group"
    if not name: return jsonify(error="اسم المجموعة مطلوب"), 400
    if len(name)>80: return jsonify(error="الاسم طويل جدًا"), 400
    try:
        r = Room(name=name, description=d.get("description","")[:500],
                 kind=kind, emoji=d.get("emoji","👥"), owner_id=uid,
                 code=uuid.uuid4().hex[:12].upper())  # FIX 6: hex no hyphens
        db.session.add(r)
        db.session.flush()
        mem = Member(room_id=r.id, user_id=uid, role="owner")
        db.session.add(mem)
        db.session.commit()
        return jsonify(_room_payload(r, uid, mem)), 201  # FIX 4: full payload
    except Exception:
        db.session.rollback()
        return jsonify(error="فشل إنشاء الغرفة، حاول مرة أخرى"), 500
...
def join_by_code():
    ...
    if not r: return jsonify(error="رمز الدعوة غير صالح"), 404
    try:
        ex = Member.query.filter_by(room_id=r.id, user_id=uid).first()
        if not ex:
            ex = Member(room_id=r.id, user_id=uid)
            db.session.add(ex)
            db.session.commit()
        return jsonify(room=_room_payload(r, uid, ex))
    except Exception:
        db.session.rollback()
        return jsonify(error="فشل الانضمام، حاول مرة أخرى"), 500
...
def start_dm(username):
    ...
    if not other: return jsonify(error="المستخدم غير موجود"), 404
    if other.id==uid: return jsonify(error="لا يمكنك بدء محادثة مع نفسك"), 400
    try:
        key = "::".join(sorted([uid, other.id]))
        r   = Room.query.filter_by(dm_key=key).first()
        if not r:
            r = Room(name=other.username, kind="dm", emoji="",
                     code=uuid.uuid4().hex[:12].upper(), dm_key=key)
            db.session.add(r)
            db.session.flush()
        ...
        db.session.rollback()
        return jsonify(error="فشل بدء المحادثة"), 500
...
def get_messages(rid):
    ...
    if not mem: return jsonify(error="غير مصرح"), 403
    try:
        ...
        db.session.rollback()
        return jsonify(error="فشل تحميل الرسائل"), 500
...
def me():
    ...
    if request.method=="PATCH":
        ...
        if "username" in d:
            ...
            if 3<=len(new)<=30 and not User.query.filter(
                    User.username==new, User.id!=uid).first():
                u.username = new
...
def s_msg(data):
    ...
    if kind not in ("text","image","voice"): return
    if kind=="text":
        text = text.strip()[:MAX_TEXT_LEN]
    elif len(text)>MAX_MEDIA_LEN:
        emit("err",{"msg":"media_too_large"}); return
    if not (uid and rid and text): return
    if not Member.query.filter_by(room_id=rid,user_id=uid).first(): return
    try:
...
def s_typing(data):
    ...
    if uid and rid:
        u = db.session.get(User, uid)
        emit("typing",{"uid":uid,"name":u.username if u else "؟",
                        "rid":rid,"on":bool(data.get("on",False))},
    ...
    if uid:
        try:
            ...
            if u:
                ...
                db.session.commit()
            emit("presence",{"uid":uid,"online":False,
                              "last_seen":datetime.utcnow().isoformat()}, broadcast=True)
        except Exception:
            db.session.rollback()
